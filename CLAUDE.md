...
6. אימות מקומי לפני שמראים לבן: הרצת שרת (`npx serve` או `python -m http.server`) plus בדיקה שהקבצים נטענים (HTTP 200), שה-frontmatter תקין, ושכל קישור-הורדה מצביע לקובץ קיים. **בלי צילומי-מסך** — אימות שאתר נבנה דרך screenshots הוא מיותר.
...
- אימות מקומי (serve plus בדיקת קבצים/HTTP/הורדות) לפני "מוכן" — בלי צילומי-מסך.
